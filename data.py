...
def categorize(labels, dict_labels=None):
    ...
    new_labels = np_utils.to_categorical(new_labels) #Converts a class vector (integers) to binary class matrix.
    return new_labels
...
class DataGenerator(object):
    ...
    def __init__(self, batch_size=32, shuffle=True):
        # Initialization
        self.batch_size = batch_size # num of samples in each round
...
def load_data(path, maxlen=None, minlen=0, traces=0, dnn_type=None, openw=False):
    ...
    npzfile = np.load(path, encoding='latin1')
    #npzfile = open(path, encoding='latin1')
    data = npzfile["data"]
    ...
    # crea el array dict_labels[] con tantos espacios como y
    dict_labels = {}
    i = 0
    for yy in clas:
        dict_labels[yy] = i
    ...
    if type == "sdae" and len(data.shape) > 2:
        print("WEIRD! dataset.shape={}".format(data.shape))
        data = data.reshape(data.shape[0], data.shape[1])

    if not openw:
        print("Categorize")
        labels = categorize(labels) #convierte en una matriz binaria
